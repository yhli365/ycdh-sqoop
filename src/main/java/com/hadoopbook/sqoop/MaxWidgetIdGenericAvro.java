...
public class MaxWidgetIdGenericAvro extends Configured implements Tool {
	...
	public static class MaxWidgetMapper
			extends
			Mapper<AvroKey<GenericRecord>, NullWritable, AvroKey<Long>, AvroValue<GenericRecord>> {
		...
		protected void map(AvroKey<GenericRecord> key, NullWritable value,
			...
			if (id != null) {
				if (maxWidget == null || id > (Integer) maxWidget.get("id")) {
					maxWidget = widget;
				}
			}
		}
		...
		protected void cleanup(Context context) throws IOException,
			...
			context.write(new AvroKey(0L), new AvroValue<GenericRecord>(
					maxWidget));
		}
	}
	...
	public static class MaxWidgetReducer
			extends
			Reducer<AvroKey<Long>, AvroValue<GenericRecord>, AvroKey<GenericRecord>, NullWritable> {
		...
		protected void reduce(AvroKey<Long> key,
			...
			for (AvroValue<GenericRecord> value : values) {
				...
				if (maxWidget == null
						|| (Integer) record.get("id") > (Integer) maxWidget
								.get("id")) {
